...
### Roadmap futura (non nel MVP)

**Multi-impianto ibrido:** molte case italiane hanno piu sistemi conviventi (pannelli radianti IR in camera + pompa di calore in salotto + caldaia a gas per acqua calda). EH gia riconosce il tipo di impianto principale, ma la versione futura gestira:
- **Device multipli con logiche diverse** — recircolo solo dove c e climatizzatore, switch solo per la pompa, setback globale
- **Fotovoltaico (8° pilastro)** — auto-consumo ottimizzato, carico/scarico rete, immissione in rete quando il PUN e alto. Richiede inverter connesso a Home Assistant e API Octopus per prezzi immissione/prelievo
- **Scalda-acqua elettrico / heat pump** — scheduling del boiler nelle fasce di prezzo minimo

Queste funzioni sono escluse dal MVP perche richiedono hardware specifico (inverter FV, contatore bidirezionale) e integrazioni API aggiuntive.
